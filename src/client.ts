...
import {
  AnalystRetrieveEstimatesResponse,
  AnalystRetrievePriceTargetsResponse,
  AnalystRetrieveRecommendationsResponse,
  Analysts,
} from './resources/analysts';
import { ChartRetrieveHistoricalParams, ChartRetrieveHistoricalResponse, Charts } from './resources/charts';
import {
  EarningGetCalendarParams,
  EarningGetCalendarResponse,
  EarningGetTranscriptsParams,
  EarningGetTranscriptsResponse,
  Earnings,
} from './resources/earnings';
import {
  EconomicGetCalendarParams,
  EconomicGetCalendarResponse,
  EconomicGetIndicatorDataParams,
  EconomicGetIndicatorDataResponse,
  Economics,
} from './resources/economics';
import { Etf, EtfRetrieveHoldingsParams, EtfRetrieveHoldingsResponse } from './resources/etf';
import {
  FinancialRetrieveBalanceSheetParams,
  FinancialRetrieveBalanceSheetResponse,
  FinancialRetrieveCashFlowParams,
  FinancialRetrieveCashFlowResponse,
  FinancialRetrieveFinancialScoreParams,
  FinancialRetrieveFinancialScoreResponse,
  FinancialRetrieveIncomeStatementParams,
  FinancialRetrieveIncomeStatementResponse,
  FinancialRetrieveKeyMetricsParams,
  FinancialRetrieveKeyMetricsResponse,
  FinancialRetrieveRatiosParams,
  FinancialRetrieveRatiosResponse,
  Financials,
} from './resources/financials';
import {
  InsiderTrading,
  InsiderTradingRetrieveParams,
  InsiderTradingRetrieveResponse,
} from './resources/insider-trading';
import {
  News,
  NewsRetrieveCryptoParams,
  NewsRetrieveCryptoResponse,
  NewsRetrieveForexParams,
  NewsRetrieveForexResponse,
  NewsRetrieveStockParams,
  NewsRetrieveStockResponse,
} from './resources/news';
import { QuoteRetrieveResponse, Quotes } from './resources/quotes';
import { Search, SearchGeneralSearchParams, SearchGeneralSearchResponse } from './resources/search';
import {
  TechnicalRetrieveIndicatorDataParams,
  TechnicalRetrieveIndicatorDataResponse,
  Technicals,
} from './resources/technicals';
import { readEnv } from './internal/utils/env';
import { formatRequestDetails, loggerFor } from './internal/utils/log';
import { isEmptyObj } from './internal/utils/values';
import { Company } from './resources/company/company';
import { Government } from './resources/government/government';
import { Market } from './resources/market/market';
import {
  OptionRetrieveQuotesParams,
  OptionRetrieveQuotesResponse,
  OptionRetrieveTechnicalIndicatorParams,
  OptionRetrieveTechnicalIndicatorResponse,
  Options,
} from './resources/options/options';

export interface ClientOptions {
  /**
   * API Key needed to access the endpoints. Obtainable after registration.
   */
   ...
   *
   * Defaults to process.env['FINBUD_DATA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['FINBUD_DATA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Finbud Data API.
 */
export class FinbudData {
  apiKey: string;
   ...
   * API Client for interfacing with the Finbud Data API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['FINBUD_DATA_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['FINBUD_DATA_BASE_URL'] ?? /api] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('FINBUD_DATA_BASE_URL'),
    apiKey = readEnv('FINBUD_DATA_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.FinbudDataError(
        "The FINBUD_DATA_API_KEY environment variable is missing or empty; either provide it, or instantiate the FinbudData client with an apiKey option, like new FinbudData({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? FinbudData.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('FINBUD_DATA_LOG'), "process.env['FINBUD_DATA_LOG']", this) ??
      defaultLogLevel;
  ...
  static FinbudData = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static FinbudDataError = Errors.FinbudDataError;
  ...
  analysts: API.Analysts = new API.Analysts(this);
  charts: API.Charts = new API.Charts(this);
  company: API.Company = new API.Company(this);
  earnings: API.Earnings = new API.Earnings(this);
  economics: API.Economics = new API.Economics(this);
  etf: API.Etf = new API.Etf(this);
  financials: API.Financials = new API.Financials(this);
  government: API.Government = new API.Government(this);
  insiderTrading: API.InsiderTrading = new API.InsiderTrading(this);
  ...
  technicals: API.Technicals = new API.Technicals(this);
}
FinbudData.Analysts = Analysts;
FinbudData.Charts = Charts;
FinbudData.Company = Company;
FinbudData.Earnings = Earnings;
FinbudData.Economics = Economics;
FinbudData.Etf = Etf;
FinbudData.Financials = Financials;
FinbudData.Government = Government;
FinbudData.InsiderTrading = InsiderTrading;
FinbudData.Market = Market;
FinbudData.News = News;
FinbudData.Options = Options;
FinbudData.Quotes = Quotes;
FinbudData.Search = Search;
FinbudData.Technicals = Technicals;
export declare namespace FinbudData {
  export type RequestOptions = Opts.RequestOptions;

  export {
    Analysts as Analysts,
  ...
  export { Government as Government };

  export {
    InsiderTrading as InsiderTrading,
