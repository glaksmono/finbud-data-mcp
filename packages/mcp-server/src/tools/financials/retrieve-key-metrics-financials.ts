...
export const tool: Tool = {
  name: 'retrieve_key_metrics_financials',
  description: 'Fetches the 10 most recent annual key metrics for a given company stock symbol.',
  inputSchema: {
    type: 'object',
    properties: {
      symbol: {
...
export const handler = async (client: FinbudData, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await client.financials.retrieveKeyMetrics(symbol, body));
};
