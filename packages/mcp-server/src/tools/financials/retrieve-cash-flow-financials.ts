...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/financials/cash-flow/{symbol}',
  operationId: 'getCashFlow',
};

export const tool: Tool = {
  name: 'retrieve_cash_flow_financials',
  description: 'Fetches the 10 most recent annual cash flow statements for a given company stock symbol.',
  inputSchema: {
    type: 'object',
    properties: {
      symbol: {
...
export const handler = async (client: FinbudData, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await client.financials.retrieveCashFlow(symbol, body));
};
