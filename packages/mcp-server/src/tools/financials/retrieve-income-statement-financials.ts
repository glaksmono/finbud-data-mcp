...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/financials/income-statement/{symbol}',
  operationId: 'getIncomeStatement',
};

export const tool: Tool = {
  name: 'retrieve_income_statement_financials',
  description: 'Fetches the 10 most recent annual income statements for a given company stock symbol.',
  inputSchema: {
    type: 'object',
    properties: {
      symbol: {
...
export const handler = async (client: FinbudData, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await client.financials.retrieveIncomeStatement(symbol, body));
};
